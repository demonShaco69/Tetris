...
def draw_text_middle(text, size, color, surface):
    font = pygame.font.Font(fontpath, size, bold=True, italic=True)
...
def draw_next_shape(piece, surface):
    ...
    start_y = top_left_y + (play_height / 2)
...
def draw_window(surface, grid, score=0):
    ...
    pygame.font.init()
    font = pygame.font.Font(fontpath, 150, bold=True)
    label = font.render('TETRIS', 1, (255, 255, 255))

    surface.blit(label, (
    (top_left_x + play_width / 2) - (label.get_width() / 2), -20))
...
def main(window):
    ...
    window.fill("BLACK")
    draw_text_middle('YOU LOST!', 60, (255, 255, 255), window)
    pygame.display.update()
